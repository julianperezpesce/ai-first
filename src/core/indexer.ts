...
export class IncrementalIndexer {
  ...
  private stats = { files: 0, symbols: 0, imports: 0 };

  constructor(rootDir: string, outputPath: string, debounceMs = 300) {
  ...
  private async loadFileHashes(): Promise<void> {
    ...
    if (result.length > 0) {
      for (const row of result[0].values) {
        this.fileHashes.set(row[0] as string, {
   ...
   * Compute file hash
   */
  private computeHash(filePath: string): { hash: string; mtime: number } | null {
    try {
  ...
  private updateFileHash(filePath: string): void {
    const current = this.computeHash(filePath);
    if (!current || !this.db) return;
    
    this.db.run(
      "INSERT OR REPLACE INTO file_hashes (path, hash, mtime) VALUES (?, ?, ?)",
      [filePath, current.hash, current.mtime]
    );
  ...
  async processFile(filePath: string): Promise<void> {
    ...
    if (existingFile.length > 0 && existingFile[0].values.length > 0) {
      fileId = existingFile[0].values[0][0] as number;
      ...
      fileId = result[0].values[0][0] as number;
    }
   ...
   */
  async watch(ignoredPatterns: string[] = []): Promise<void> {
 ...
 */
function parseFileForSymbols(filePath: string, extension: string): { name: string; type: string; line: number; exported: boolean }[] {
...
function parsePython(lines: string[], symbols: { name: string; type: string; line: number; exported: boolean }[]): void {
  for (let i = 0; i < lines.length; i++) {
    ...
    if (classMatch) symbols.push({ name: classMatch[1], type: "class", line: i + 1, exported: true });
    const funcMatch = line.match(/^def\s+(\w+)/);
    if (funcMatch) symbols.push({ name: funcMatch[1], type: "function", line: i + 1, exported: true });
  }
}
...
function parseGo(lines: string[], symbols: { name: string; type: string; line: number; exported: boolean }[]): void {
  for (let i = 0; i < lines.length; i++) {
    ...
    if (typeMatch) symbols.push({ name: typeMatch[1], type: "type", line: i + 1, exported: true });
  }
}
...
function parseJava(lines: string[], symbols: { name: string; type: string; line: number; exported: boolean }[]): void {
  for (let i = 0; i < lines.length; i++) {
    ...
    if (classMatch) symbols.push({ name: classMatch[1], type: "class", line: i + 1, exported: line.startsWith("public") });
    const interfaceMatch = line.match(/^(?:public\s+)?interface\s+(\w+)/);
    if (interfaceMatch) symbols.push({ name: interfaceMatch[1], type: "interface", line: i + 1, exported: line.startsWith("public") });
  }
}
...
function parseSwift(lines: string[], symbols: { name: string; type: string; line: number; exported: boolean }[]): void {
  for (let i = 0; i < lines.length; i++) {
    ...
    if (funcMatch) symbols.push({ name: funcMatch[1], type: "function", line: i + 1, exported: line.startsWith("public") });
  }
}
...
function parseRust(lines: string[], symbols: { name: string; type: string; line: number; exported: boolean }[]): void {
  for (let i = 0; i < lines.length; i++) {
    ...
    if (structMatch) symbols.push({ name: structMatch[1], type: "struct", line: i + 1, exported: true });
  }
}
 ...
 */
function parseFileForImports(filePath: string, extension: string): { target_file: string; type: string }[] {
...
function computeFileHash(filePath: string): { hash: string; mtime: number } | null {
  try {
    const stats = fs.statSync(filePath);
    const content = fs.readFileSync(filePath);
    const hash = crypto.createHash("md5").update(content).digest("hex");
...
export async function generateIndex(rootDir: string, outputPath: string): Promise<IndexResult> {
  try {
    ...
    for (const filePath of allFiles) {
      ...
      const symbols = parseFileForSymbols(filePath, extension);
      for (const sym of symbols) {
        db.run("INSERT INTO symbols (name, type, file_id, line, exported) VALUES (?, ?, ?, ?, ?)",
          [sym.name, sym.type, fileId, sym.line, sym.exported ? 1 : 0]);
        symbolCount++;
      }
      ...
      const imports = parseFileForImports(filePath, extension);
      for (const imp of imports) {
        db.run("INSERT INTO imports (source_file_id, target_file, type) VALUES (?, ?, ?)",
          [fileId, imp.target_file, imp.type]);
        importCount++;
      }
      ...
      if (hashData) {
        db.run("INSERT INTO file_hashes (path, hash, mtime) VALUES (?, ?, ?)",
          [relativePath, hashData.hash, hashData.mtime]);
      }
    }
