...
import { loadSymbolGraph, SymbolGraph, SymbolRelationship } from "./symbolGraph.js";
import { generateSymbolId } from "../analyzers/symbols.js";
...
export function generateContextPacket(
  ...
  try {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      // Add line numbers
      sourceCode = lines.slice(startLine, endLine).map((line, i) => 
        `${startLine + i + 1}: ${line}`
      ).join("\n");
    }
  } catch {
