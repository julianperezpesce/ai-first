...
export async function generateModuleGraph(
  ...
  for (const file of files) {
    ...
    if (parts[0] === "ai" || parts[0] === "node_modules" || parts[0] === ".git") continue;
    if (parts[0] === "dist" || parts[0] === "build" || parts[0] === "coverage") continue;
  ...
  const graph: ModuleGraph = { modules, dependencies };
 ...
 */
function parseFileImports(file: FileInfo, rootDir: string): string[] {
 ...
 */
function getModuleFromPath(importPath: string): string | null {
 ...
 */
export function loadModuleGraph(aiDir: string): ModuleGraph | null {
