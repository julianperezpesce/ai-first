 ...
 */
function generateRepoMapJson(files: FileInfo[]): string {
 ...
 */
function generateSymbolsJson(files: FileInfo[]): string {
 ...
 */
function extractJsTsSymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
 ...
 */
function extractPythonSymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    const lineNum = i + 1;
 ...
 * Extract Go symbols
 */
function extractGoSymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    const lineNum = i + 1;
 ...
 */
function extractJavaSymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    const lineNum = i + 1;
 ...
 */
function extractRubySymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    const lineNum = i + 1;
 ...
 * Extract PHP symbols
 */
function extractPHPSymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    const lineNum = i + 1;
 ...
 */
function extractSwiftSymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    const lineNum = i + 1;
 ...
 */
function extractKotlinSymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    const lineNum = i + 1;
 ...
 */
function extractRustSymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    const lineNum = i + 1;
 ...
 */
function extractApexSymbols(lines: string[], filePath: string, symbols: SymbolEntry[]): void {
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    const lineNum = i + 1;
 ...
 */
function generateDependenciesJson(files: FileInfo[]): string {
  ...
  const byFile: Record<string, DependencyEntry[]> = {};
  for (const dep of dependencies) {
    if (!byFile[dep.source]) byFile[dep.source] = [];
 ...
 */
function generateAIContextMarkdown(files: FileInfo[], rootDir: string): string {
  const lines: string[] = [];
 ...
 */
function getExtensionStats(files: FileInfo[]): Record<string, number> {
 ...
 */
function buildSimpleTree(files: FileInfo[]): string {
 ...
 * Find entry points
 */
function findEntrypoints(files: FileInfo[]): { path: string; description: string }[] {
  const entryPoints: { path: string; description: string }[] = [];
 ...
 */
function detectConventions(files: FileInfo[]): string[] {
 ...
 * Detect key modules
 */
function detectModules(files: FileInfo[]): { name: string; description: string; fileCount: number }[] {
  const modules: { name: string; description: string; fileCount: number }[] = [];
 ...
 */
function detectArchitectureHints(files: FileInfo[]): string[] {
  const hints: string[] = [];
 ...
 */
function countSymbols(jsonStr: string): number {
  try {
 ...
 */
function countDependencies(jsonStr: string): number {
  try {
    const data = JSON.parse(jsonStr);
    return data.total || 0;
  } catch {
