...
export interface ChunkOptions {
  ...
  overlap?: number;        // Lines of overlap between chunks
}
 ...
 */
function chunkJS(content: string, filePath: string, options: ChunkOptions): Chunk[] {
  const chunks: Chunk[] = [];
  ...
  const boundaries: { line: number; name?: string; type: "function" | "class" }[] = [];
 ...
 */
function chunkPython(content: string, filePath: string, options: ChunkOptions): Chunk[] {
  const chunks: Chunk[] = [];
  const lines = content.split("\n");
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const p of patterns) {
      if (p.regex.test(line)) {
        boundaries.push({ line: i + 1, type: p.type as 'function' | 'class' });
        break;
      }
    }
  }
 ...
 */
function chunkGo(content: string, filePath: string, options: ChunkOptions): Chunk[] {
  const chunks: Chunk[] = [];
  const lines = content.split("\n");
  const language = "go";
  const maxSize = options.maxChunkSize || 2000;
 ...
 */
function chunkBySize(content: string, filePath: string, options: ChunkOptions): Chunk[] {
  const chunks: Chunk[] = [];
  const lines = content.split("\n");
  const maxSize = options.maxChunkSize || 2000;
 ...
 */
export function chunkFiles(filePaths: string[], options: ChunkOptions = {}): Chunk[] {
