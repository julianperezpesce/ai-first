...
function collectModuleDirectories(files: FileInfo[], directories: string[]): string[] {
...
function inferModuleResponsibility(dir: string, extensions: string[]): string {
  ...
  const knownResponsibilities: Record<string, string> = {
...
function primaryName(name: string): string {
  if (name.includes("github")) return "Repository automation";
  if (name.includes("netlify")) return "Deployment configuration";
...
function analysisIsCli(analysis: ArchitectureAnalysis): boolean {
