...
export async function runDoctor(rootDir: string, fixMode: boolean): Promise<DoctorResult> {
  ...
  const largeFiles = scanResult.files.filter(f => { try { return fs.statSync(f.path).size > 1024 * 1024; } catch { return false; } });
