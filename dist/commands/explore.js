...
export async function runExplore(rootDir, moduleName) {
    ...
    if (!moduleName || moduleName === 'all') {
        ...
        if (fs.existsSync(graphFile)) {
            ...
            return { success: true, modules: data.modules?.map((m) => m.name) };
        }
        const scan = scanRepo(rootDir);
        const modules = new Set();
        for (const f of scan.files) {
            const p = f.relativePath.split('/');
            if (p.length > 1 && p[0] !== 'ai' && p[0] !== 'node_modules')
                modules.add(p[0]);
        }
    ...
    if (fs.existsSync(graphFile)) {
        ...
        if (mod) {
            ...
            for (const p of mod.paths || [])
                console.log(`  ${p}`);
            const deps = data.dependencies?.filter((d) => d.from === moduleName || d.to === moduleName) || [];
            if (deps.length > 0) {
                console.log("\nDependencies:");
                for (const d of deps)
                    console.log(`  ${d.from} → ${d.to}`);
            }
            return { success: true, dependencies: deps.map((d) => d.to === moduleName ? d.from : d.to) };
        }
        return { success: false, error: `Module '${moduleName}' not found` };
    }
    return { success: false, error: 'Run: ai-first explore all' };
}
export function exploreMain(args) {
