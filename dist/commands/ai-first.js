...
import { detectGitRepository, generateGitContext, getRecentCommits } from "../core/gitAnalyzer.js";
...
function installSynchronousConsoleForPipedOutput() {
...
function printFreshnessReport(rootDir, outputDir, jsonMode) {
    ...
    if (freshness.generatedAt)
        console.log(`Generated: ${freshness.generatedAt}`);
...
function printVerificationReport(rootDir, outputDir, jsonMode) {
...
`);
        ...
        query = query.trim();
        if (!query) {
            console.log("Usage: af search <query> [--root dir]");
            process.exit(1);
        }
        ...
        if (!question) {
            console.log("Usage: af ask <question> [--root dir]");
            process.exit(1);
        }
        console.log(`\n🧠 "${question}"\n`);
        const { semanticSearch } = await import("../utils/semanticSearch.js");
        ...
        topic = topic.trim();
        if (!topic) {
            console.log("Usage: af understand <topic> [--root dir]");
            process.exit(1);
        }
        console.log(`\n🧠 Understanding: "${topic}"\n`);
        const { semanticSearch } = await import("../utils/semanticSearch.js");
        ...
        const ctxPath = path.join(rootDir, "ai-context", "ai_context.md");
        if (fs.existsSync(ctxPath))
            console.log(fs.readFileSync(ctxPath, "utf-8").match(/## Cross-Cutting.*?\n([\s\S]*?)\n---/)?.[1]?.trim() || "Run af init for full context");
        console.log("\n## Related Files");
        ...
        const prompt = () => {
            rl.question("af> ", (input) => {
                ...
                if (input === "/help") {
                    console.log("/exit quit | /help | /context show context\nAsk any question about your codebase");
                    prompt();
                    return;
                }
                if (input === "/context") {
                    const cp = path.join(rootDir, "ai-context", "ai_context.md");
                    if (fs.existsSync(cp))
                        console.log(fs.readFileSync(cp, "utf-8").split("\n").slice(0, 20).join("\n"));
                    else
                        console.log("Run af init first");
                    prompt();
                    return;
                }
                if (input.startsWith("/")) {
