...
function generateUnifiedContext(repoMap, summary, architecture, techStack, entrypoints, conventions, aiRules, projectSetup, depVersions, testMapping, dataModels, recentChanges, crossCutting, configAnalysis, gotchas) {
    ...
    if (configAnalysis) {
        const configs = [];
        if (configAnalysis.typescript)
            configs.push(`TypeScript: strict=${configAnalysis.typescript.strict}, target=${configAnalysis.typescript.target}`);
        if (configAnalysis.eslint)
            configs.push(`ESLint: ${configAnalysis.eslint.extends.join(", ") || "custom"}`);
        if (configAnalysis.prettier)
            configs.push(`Prettier: semi=${configAnalysis.prettier.semi}, singleQuote=${configAnalysis.prettier.singleQuote}`);
        if (configAnalysis.testing)
            configs.push(`Testing: ${configAnalysis.testing.framework}`);
        if (configAnalysis.docker?.hasDockerfile)
            configs.push(`Docker: ${configAnalysis.docker.baseImage || "Dockerfile"}`);
        if (configs.length > 0) {
            lines.push("---\n");
