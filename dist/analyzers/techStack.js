...
function collectTechStackEvidence(input) {
    ...
    const addPackageEvidence = (category, names, fallbackConfidence) => {
...
function detectLanguages(extensions) {
    ...
    const primaryLangs = langs.filter(l => !["YAML", "JSON", "XML", "Markdown", "Shell", "Bash", "Docker", "CSS", "SCSS", "LESS"].includes(l));
    if (primaryLangs.length > 0) {
        // Add Markdown for docs, JSON for config files
        const allLangs = [...new Set(primaryLangs)];
        if (extensions.has("md") || extensions.has("mdx"))
            allLangs.push("Markdown");
...
function detectTesting(files, fileNames) {
    ...
    try {
        ...
        if (pkgFile) {
            const pkg = readJsonFile(pkgFile.path);
            const deps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) };
            const scripts = pkg.scripts || {};
