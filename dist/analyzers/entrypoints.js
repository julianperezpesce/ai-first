...
function enrichEntrypointEvidence(entrypoint) {
...
function discoverSalesforceEntrypoints(files, rootDir) {
    const entrypoints = [];
        ...
        try {
            ...
            if (apexFile.extension === "trigger") {
                ...
                if (classMatch) {
                    ...
                    else if (auraEnabled.length > 0)
                        description += ` (${auraEnabled.length} @AuraEnabled methods)`;
                    else if (webservice)
...
function extractApexMethods(content) {
    const methods = [];
...
function discoverSwiftUIEntrypoints(swiftFiles, rootDir) {
    const entrypoints = [];
...
function discoverGoEntrypoints(goFiles, rootDir) {
    const entrypoints = [];
...
function discoverRustEntrypoints(rustFiles, rootDir) {
    const entrypoints = [];
...
function discoverPHPEntrypoints(phpFiles, rootDir) {
    const entrypoints = [];
