...
**Trusted AI Context** - Fresh, evidence-backed context for agents and CI

- ✅ `af verify ai-context`: Context Truth Score with pass/warn/fail checks
- ✅ `af doctor context`: Freshness check for generated `ai-context/`
- ✅ `ai-context/context_manifest.json`: generatedAt, version, git state, file hashes
- ✅ `ai-context/agent_brief.md`: short operational brief for coding agents
- ✅ Evidence/confidence metadata in tech stack, entrypoints, config, test mapping, security, performance, and dead-code output
- ✅ MCP tools for agents: `get_project_brief`, `is_context_fresh`, `run_doctor`, `verify_ai_context`
- ✅ Task-specific context v2: relevant files, tests, commands, risks, contracts, confidence, and evidence
- ✅ Quality gates for CI/agents with `af doctor --ci` and MCP `get_quality_gates`
- ✅ MCP compatibility profiles for OpenCode, Codex, Claude Code, Cursor, and generic stdio clients
- ✅ MCP Streamable HTTP transport for URL-based agent clients
- ✅ HTTP MCP hardening with bearer token support and unsafe bind protection
...
**For AI agents:** use the [AI Agent Playbook](./docs/guide/ai-agent-playbook.md) to decide when to verify context, when to regenerate it, and which MCP/CLI tool to call for each task.

**Previous: v1.5.0**

**Semantic Search & Code Understanding** - Inspired by Graphify, ChromaDB & FAISS

- ✅ `af search "query"`: Keyword-based code search across the entire repo
- ✅ `af ask "question"`: Answers questions with context + code snippets
- ✅ `af understand "topic"`: Context + graph + code + tests in one response
- ✅ `af chat`: Interactive REPL mode for codebase questions
...
| `af context --task <task>` | Generate task-specific context for agent workflows |
...
| `af doctor --ci` | Run quality gates for CI and AI agents |
| `af doctor context` | Check whether generated AI context is fresh |
| `af verify ai-context` | Audit generated context with Context Truth Score |
...
├── agent_brief.md     # Short brief for AI coding agents
├── context_manifest.json # Freshness metadata, git state, file hashes
...
For generated context trust checks:
...
`af verify ai-context` returns a Context Truth Score from 0 to 100 and exits non-zero when the generated context is stale, missing, or materially inconsistent.

`af doctor --ci` evaluates quality gates such as package bin validity, build/test scripts, TypeScript config, CI workflow coverage, MCP shell safety, docs build script and context trust. Use `--output` when CI generates context outside the default `ai-context/` directory.
